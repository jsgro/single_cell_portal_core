class AddCrossDatasetSearchFlag < Mongoid::Migration
  ...
  def self.up
    FeatureFlagMigrator.create!(name: 'cross_dataset_search',
                                default_value: false,
                                description: 'whether home page uses cross dataset search UI')
  end

  def self.down
    FeatureFlagMigrator.find_by(name: 'cross_dataset_search').destroy
  end
end
