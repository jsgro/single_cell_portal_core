class AddCrossDatasetSearchFrontendFlag < Mongoid::Migration
  ...
  def self.up
    FeatureFlagMigrator.create!(name: 'cross_dataset_search_frontend',
                                default_value: false,
                                description: 'whether home page uses cross dataset search UI')
  end

  def self.down
    FeatureFlagMigrator.find_by(name: 'cross_dataset_search_frontend').destroy
  end
end
