# module that allows any class that is primarily utilized via Delayed::Job to retrieve job instances of itself from
# the Delayed::Job queue

module DelayedJobAccessor
  extend ActiveSupport::Concern

  ALLOWED_JOB_TYPES = [DeleteQueueJob, IngestJob, UploadCleanupJob]

  class_methods do
    # find a Delayed::Job instance of a particular class, and refine by an associated object
    #
    # * *params*
    #   - job_class (Class) => Class of job to find, such as UploadCleanupJob, DeleteQueueJob, or IngestJob
    ...
    def find_jobs_by_handler_type(job_class, associated_object)
      matching_jobs = []
    ...
    def match_handler_to_object(handler, job_class, associated_object)
