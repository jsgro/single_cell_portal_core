...
class StudyFileTest < ActiveSupport::TestCase
  ...
  test 'should validate user-uploaded differential expression file' do
    ...
    FactoryBot.create(:metadata_file, name: 'metadata.txt', study:, cell_input: cells, annotation_input: [
                        { name: 'cell_type__ontology_label', type: 'group', values: cell_types },
                        { name: 'cell_type__custom', type: 'group', values: custom_cell_types }
                      ])
    FactoryBot.create(:cluster_file, name: 'cluster.txt', study:, cell_input: { x: coords, y: coords, cells: })
