...
describe('Download selection modal', () => {
  ...
  it('shows the correct total size, and generates a command with selected file ids', async () => {
    ...
    fetchAuthCode.mockImplementation(() => Promise.resolve(
      { authCode: 'TViAHJmA', timeInterval: 1800 }
    ))
    ...
    await waitForElementToBeRemoved(() => screen.getByTestId('bulk-download-loading-icon'))
    expect(screen.getByRole('textbox')).toHaveValue('curl "undefined/single_cell/api/v1/bulk_download/generate_curl_config?auth_code=TViAHJmA&file_ids=60403d30cc7ba03f94477640,60a2cf62cc7ba082358b545f,60a2cf63cc7ba082358b5464,60a2cf64cc7ba082358b5466,60a5ba1fcc7ba0360a9ac24f,60a5ba1fcc7ba0360a9ac253,60a5ba20cc7ba0360a9ac255" -o cfg.txt; curl -K cfg.txt && rm cfg.txt')
  })
