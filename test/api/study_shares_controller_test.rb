...
class StudySharesControllerTest < ActionDispatch::IntegrationTest
    ...
    @study_share.attributes.each do |attribute, value|
      if attribute =~ /_id/
        assert json[attribute] == JSON.parse(value.to_json),
               "Attribute mismatch: #{attribute} is incorrect, expected #{JSON.parse(value.to_json)} but found #{json[attribute.to_s]}"
      elsif attribute =~ /_at/
  ...
  test 'should create then update then delete study share' do
    ...
    assert json['email'] == study_share_attributes[:study_share][:email],
           "Did not set email correctly, expected #{study_share_attributes[:study_share][:email]} but found #{json['email']}"
    # update study share
    ...
    assert json['deliver_emails'] == update_attributes[:study_share][:deliver_emails],
           "Did not set deliver_emails correctly, expected #{update_attributes[:study_share][:deliver_emails]} but found #{json['deliver_emails']}"
    # delete study share
